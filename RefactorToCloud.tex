...
In addition, we need to make a choice between using the cloud (i) as a backup for the data for one user across multiple devices(``just me session'') or (ii) to enable collaboration between multiple users across multiple devices (``everyone session'').  \TODO{HOW MANY LOC IS THIS CHANGE}
...
Table\TODO{TABLE} provides details about our corpus.  We report the size in LOC and complexity in number of functions. 
...
\end{center}
\end{table}%
\TODO{ADD TO TOTAL, call it \# Call Sites, sort by total calls}
