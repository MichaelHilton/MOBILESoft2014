...
\documentclass[article]{sigplanconf}
...
%\conferenceinfo{MOBILESoft}{'14, June 2–3, 2014, Hyderabad, India}
%\CopyrightYear{14}
...
\category{D.2.7}{\CR{Software Engineering}}{\CR{Distribution, Maintenance, and Enhancement}}

\terms{\CR{Refactoring}}

\keywords{\CR{Refactoring, Cloud Computing, Empirical Study}}
...
\CR{\url{http://cope.eecs.oregonstate.edu/cloudifier}}
...
The first app we selected was \MT, which is publicly available 
\CR{(the script id can be found at~\cite{cloudifierData})}.  Mileage Tracker is an app that records and calculates fuel usage in Miles per Gallon (MPG) and displays how it changes over time. \CR{We began by adding some functionality to the original app. We then converted} Milage Tracker into a multi-user app, such that multiple family members using the same family car can collect the fuel usage even across multiple devices.

Business Manager+\CR{~\cite{cloudifierData}} is an app to track business contacts.  We converted it into a multi-user app so that business colleagues can share their contact list with each other.

CliffHangers\CR{~\cite{cloudifierData}} is a clone of the popular game ``hangman."  The player is presented with a series of blanks, and they must guess the letters that fill in the blanks to make a word.  The player has a limited number of guesses, and if they cannot guess the word before they reach the limit, they \CR{lose} the game.  We converted it into a multi-player game so that two players can collaboratively work together on separate devices to guess the word.

MyAssignment\CR{~\cite{cloudifierData}} is an app for students to track their class assignments or projects.  A single user can input information about projects, progress, due dates, etc.  \CR{We converted it into} a multi-user app so that multiple students can collaborate on class assignments.
...
\CR{It took the second author an average of 6 minutes to perform steps 1 and 4 for each app in the formative study.}
...
\CR{So far, we implemented \tool only for \NC{}s.  In order to add other types of collections, we would need to write new injected functions in order to provide additional functionality.  
For example, in order to support the \code{String Collection}, we would need to write an equivalent
injected function 
for the \code{join} API method which concatenates the entire collection into one string.}
...
Table~\ref{table:UsageOfAPIS} shows the total usage of the \NC APIs in our corpus. \CR{ For each API method we report (i) the total number of call sites in our corpus, 
and (ii) the percentage of call sites among the number of \NC API calls.}
...
We ran the refactoring in batch mode. \CR{Steps 1 and 4 of our process require domain knowledge.  For Step 1, we refactored all the \NC{}s in our corpus instead of trying to identify which data makes
sense to be moved to the cloud.  
For Step 4, we used the same policy that the app currently uses for data initialization, as changing that would involve making design decisions for each app.  As far as the data session, we used the default session provided by \TD, the ``just me session.'' 
   }
...
%This review was written by Eli Tilevich (tilevich@cs.vt.edu). I've deliberately unblinded my review, as this work is closely related to my recent research on cloud refactoring. Having studied a general version of this problem for many years, I believe that this paper represents an important evolutionary step in enabling centralized programs with distributed execution capabilities. This work is well motivated, executed, and evaluated. I recommend that it be accepted for presentation.
%
%What is missing from the paper is a discussion section that would put this work into perspective. What I find most interesting about Cloudifyer is that this is the first successful example of an automated distribution approach that works by replacing types. Several similar prior approaches in managed languages, such as Java and C#, have failed to achieve this objective. Specifically, converting regular interfaces to Remote interfaces in Java RMI; and marking regular methods as [Remote] in C# Remoting. There are two reasons why Cloudifyer succeeds where the prior approaches failed. First, the cloud types in touchdevelop have a well-defined but constrained semantics that includes a sophisticated built-in eventual consistency mechanism. Second, the touchdevelop language lacks the features that make automated distribution such a hard problem. While the first reason is well-explained in Section 2.1, the second one needs to be explained further. 
%
%What makes automated distribution fiendishly hard is the possibility of aliasing. Say you have a local reference that you want to convert into a remote reference by changing its type. If there are other aliases to the local reference, which can be used to access the object directly after the distribution took place, the application will crash. Another very hard problem is the possibility of local objects accessing native libraries directly (e.g., using Java JNI). If these objects are moved to the cloud, the distributed application will also crash. There is a whole other set of other difficulties, including the differences in parameter passing semantics in local and distributed methods and different threading/synchronization semantics between local and distributed executions. To address these difficulties in J-Orchestra, I ended up having to implement a distributed shared memory via a combination of code transformation/generation. It seems that because the semantics of the tou
%chdevelop language is constrained, you are able to make safe assumptions that enable automated distribution based on replacing types. It would be informative to discuss how the design of touchdevelop makes it possible to make these assumptions.
%
%Finally, Cloudifyer is an important first step in cloud-enabling touchdevelop applications. I believe that touchdevelop can enable even a greater degree of automation of this task, without for example the user having to identify the specific types to refactor at the source code level. For example, the user could select a desired specific synchronization semantics visually, with a tool automatically generating the needed transformations. In other words, for general-purpose, managed languages such as Java, automated approaches with the programmer involved (e.g., as in my own J-Orchestra system) is the only viable choice. For special-purpose languages such as touchdevelop, it should be possible to infer and perform many refactoring transformations without exposing the end-user to any implementation details.


\CR{There are two reasons why \TD is well suited for refactoring to the cloud.  The first is that cloud types in \TD have well-defined but constrained semantics which includes the eventual consistency model as described in section~\ref{sec:ConsistencyModel}.  The second is that \TD does not allow \NC data objects to be aliased. }

\CR{In order to generalize our results beyond \TD, one must deal with the problem of aliasing.  This is a problem when the object that is being moved is referenced by another alias in the code.
Consider a local object that one wishes to convert into a cloud data type.  
If there are aliases to the local reference, any subsequent method calls via that alias will cause the application to crash, unless one finds all aliases and changes their data type as well. 
The reason this is not a concern for our tool is that \TD only allows cloud data types to be global objects, therefore ensuring there are no aliases to the object that was moved to the cloud.}
%\CR{Also, concerns must be taken to deal with local objects accessing native libraries directly.  This can also be a problem once objects are moved to the cloud.}

\CR{In order to deal with these issues in environments that allow aliases (e.g., in Java), 
researchers~\cite{Tilevich:2009} have been forced to implement distributed shared memory via a combination of code transformation/generation. }
...
\bibitem[clo()]{cloudifierData}
Refactoring to cloud data.
\newblock March '14, \url{http://cope.eecs.oregonstate.edu/cloudifier/}.

\bibitem[Badame and Dig(2012)]{badame2012refactoring}
S.~Badame and D.~Dig.
...
\bibitem[Balaban et~al.(2005)Balaban, Tip, and Fuhrer]{BalabanTF05}
...
\bibitem[Brewer(2000)]{cap-brewer}
E.~A. Brewer.
...
\bibitem[Burckhardt et~al.(2012{\natexlab{a}})Burckhardt, F\"ahndrich, Leijen,
  and Sagiv]{burckhardt2012ect}
S.~Burckhardt, M.~F\"ahndrich, D.~Leijen, and M.~Sagiv.
\newblock {E}ventually {C}onsistent {T}ransactions.
...
\bibitem[Burckhardt et~al.(2012{\natexlab{b}})Burckhardt, F{\"a}hndrich,
  Leijen, and Wood]{burckhardt2012cloud}
...
\bibitem[Chun et~al.(2011)Chun, Ihm, Maniatis, Naik, and
  Patti]{chun2011clonecloud}
...
\bibitem[Cohn(2008)]{cohn2008citizen}
J.~P. Cohn.
\newblock Citizen science: Can volunteers do real research?
\newblock \emph{BioScience}, 58\penalty0 (3):\penalty0 192--197, 2008.

\bibitem[Cooper(2010)]{cooper2010design}
S.~Cooper.
\newblock The design of alice.
\newblock \emph{ACM Transactions on Computing Education (TOCE)}, 10\penalty0
  (4):\penalty0 15, 2010.

\bibitem[Cuervo et~al.(2010)Cuervo, Balasubramanian, Cho, Wolman, Saroiu,
  Chandra, and Bahl]{cuervo2010maui}
...
\bibitem[Dig et~al.(2009)Dig, Marrero, and Ernst]{dig2009refactoring}
...
\bibitem[Flinn et~al.(2002)Flinn, Park, and Satyanarayanan]{flinn2002balancing}
...
\bibitem[Forlines et~al.(2006)Forlines, Esenther, Shen, Wigdor, and
  Ryall]{Forlines:geospatial}
...
\bibitem[Gilbert and Lynch(2002)]{gilbert-lynch-SIGACT02}
S.~Gilbert and N.~Lynch.
\newblock Brewer's conjecture and the feasibility of consistent, available,
  partition-tolerant web services.
\newblock \emph{SIGACT News}, 33:\penalty0 51--59, June 2002.

\bibitem[Gyori et~al.(2013)Gyori, Franklin, Dig, and
  Lahoda]{Gyori:Lambdaficator}
...
\bibitem[Khan et~al.()Khan, Othman, Madani, and Khan]{khan2013survey}
A.~Khan, M.~Othman, S.~Madani, and S.~Khan.
\newblock A survey of mobile cloud computing application models.

\bibitem[K{\"o}lling(2010)]{kolling2010greenfoot}
M.~K{\"o}lling.
\newblock The greenfoot programming environment.
\newblock \emph{ACM Transactions on Computing Education (TOCE)}, 10\penalty0
  (4):\penalty0 14, 2010.

\bibitem[Kwon and Tilevich(2012)]{kwon2012energy}
...
\bibitem[Kwon and Tilevich(2013)]{kwon2013cloud}
Y.-W. Kwon and E.~Tilevich.
...
\bibitem[Leichtenstern and Andr{\'e}(2009)]{Leichtenstern:multiuserGames}
...
\bibitem[Lewis et~al.(2009)Lewis, Smith, Bass, and Myers]{lewis2009report}
G.~Lewis, D.~Smith, L.~Bass, and B.~Myers.
\newblock Report of the workshop on software engineering foundations for
  end-user programming.
\newblock \emph{ACM SIGSOFT Software Engineering Notes}, 34\penalty0
  (5):\penalty0 51--54, 2009.

\bibitem[Ling et~al.(2010)Ling, Zhou, and Zheng]{ling2010refactoring}
...
\bibitem[Lopez-Gulliver et~al.(2004)Lopez-Gulliver, Tochigi, Sato, Suzuki, and
  Hagita]{Lopez-Gulliver:imageprocessing}
...
\bibitem[Maloney et~al.(2010)Maloney, Resnick, Rusk, Silverman, and
  Eastmond]{maloney2010scratch}
J.~Maloney, M.~Resnick, N.~Rusk, B.~Silverman, and E.~Eastmond.
\newblock The scratch programming language and environment.
\newblock \emph{ACM Transactions on Computing Education (TOCE)}, 10\penalty0
  (4):\penalty0 16, 2010.

\bibitem[Nacenta et~al.(2012)Nacenta, Jakobsen, Dautriche, Hinrichs, D\"{o}rk,
  Haber, and Carpendale]{Nacenta:2012:LMM:2307798.2307816}
...
\bibitem[Nguyen et~al.(2010)Nguyen, Nguyen, Wilson, Nguyen, Kim, and
  Nguyen]{Nguyen2010}
...
\bibitem[S{\o}rensen and Kjeldskov(2012)]{Sorensen:2012:ISM:2399016.2399094}
...
\bibitem[Strauch et~al.(2013)Strauch, Andrikopoulos, and
  Bachmann]{strauchmigrating}
...
\bibitem[Su and Flinn(2005)]{su2005slingshot}
Y.-Y. Su and J.~Flinn.
...
\bibitem[Terry et~al.(1995)Terry, Theimer, Petersen, Demers, Spreitzer, and
  Hauser]{terry-et-al-SIGOPS95}
...
\newblock Managing update conflicts in bayou, a weakly connected replicated
  storage system.
\newblock \emph{SIGOPS Oper. Syst. Rev.}, 29:\penalty0 172--182, December 1995.
\newblock ISSN 0163-5980.

\bibitem[Tilevich and Smaragdakis(2009)]{Tilevich:2009}
E.~Tilevich and Y.~Smaragdakis.
\newblock J-orchestra: Enhancing java programs with distribution capabilities.
\newblock \emph{ACM Trans. Softw. Eng. Methodol.}, 19\penalty0 (1):\penalty0
  1:1--1:40, Aug. 2009.
\newblock ISSN 1049-331X.

\bibitem[Tillmann et~al.(2011)Tillmann, Moskal, de~Halleux, and
  Fahndrich]{Tillmann2011TPC20482372048245}
...
\bibitem[Wloka et~al.(2009{\natexlab{a}})Wloka, Sridharan, and
  Tip]{Wloka:Reentrancer}
...
\bibitem[Wloka et~al.(2009{\natexlab{b}})Wloka, Sridharan, and
  Tip]{wloka2009refactoring}
J.~Wloka, M.~Sridharan, and F.~Tip.
\newblock Refactoring for reentrancy.
...
\bibitem[Yuill and Rogers(2012)]{Yuill:multiuserColab}
N.~Yuill and Y.~Rogers.
\newblock Mechanisms for collaboration: A design and evaluation framework for
  multi-user interfaces.
\newblock \emph{ACM Trans. Comput.-Hum. Interact.}, 19\penalty0 (1):\penalty0
  1:1--1:25, May 2012.
