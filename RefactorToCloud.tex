...
Direct Transformations are the kind of transformation where both the \NC{} and \CDT have the same API method names.  For example, both have a method "post to wall" which will, as expected, post to the script's wall.  This is the simplest of the transformations, as all that is needed to do is replace the object that the method is being called on.  Figure~\ref{fig:codeTransformations} line 4 shows code that shows a direct transformation.
...
\textbf{EFFORT}:  
Table~\ref{table:avgTrans} shows the number of transformations performed per each refactoring. \TODO{rorate the table and update this text}The first column tabulates the total number of 
