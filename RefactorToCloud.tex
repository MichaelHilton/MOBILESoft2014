...
We ran the refactoring in batch mode. \CR{Steps 1 and 4 of our process require domain knowledge.  For Step 1, we refactored all the \NC{}s in our corpus instead of trying to identify which data should be moved to the cloud.  For Step 4, we maintained the current data initialization, as changing that would involve making design decisions for each app.  As far as the data session, we used the default session provided by \TD, the ``just me session.'' 
   }
...
\CR{In order to generalize our results beyond \TD, one must deal with the problem of aliasing.  This is a problem when the object that is being moved 
Consider a local object that you wish to convert into a cloud data type.  If there are aliases to the local reference, any subsequent call to that alias will cause the application to crash.  The reason this is not a concern for our tool is that \TD only allows cloud data types to be global objects, therefore ensuring there are no aliases to the object that was moved to the cloud.}
\CR{Also, concerns must be taken to deal with local objects accessing native libraries directly.  This can also be a problem one objects are moved to the cloud.}

\CR{In order to deal with these issues in a Java, researchers have been forced to implement distributed shared memory via a combination of code transformation/generation. ~\cite{Tilevich:2009}}
