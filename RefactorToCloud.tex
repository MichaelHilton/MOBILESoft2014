...
One example of an Indirect Transformation is shown in Figure~\ref{fig:codeTransformations}.  The function \code{AddMielageToRecord} is a very simple function that adds a number to the \code{MileageRecord} \NC{}. Line 22 in column (a) shows \code{Mileage} being added via the API call \code{add}.
...
This becomes a two step process now. Step 1 is to add a new row to the \CDT, and step two is to assign the value of \code{Mileage} to that row. This is shown in column (b) lines 22-23.
...
Figure~\ref{fig:codeTransformations} shows an example of this kind of transformation.  The function \code{DisplayAvgMileage} calculates the average value of all the contents of a \NC{}, and then displays it, as seen in column (a) on lines 27-29.  Once the program has been transformed to use a \CDT to store the data instead of a \NC, there is no longer an API call that will calculate the average value of all the contents of a \CDT. Instead, \tool must introduce the custom function \code{MileageRecordTable avg} that will compute the average of all the values in the \code{MileageRecordTable}.  This is shown in column (b) lines 32-41.  One that function exists, then the API call in \code{DisplayAvgMileage} can be transformed to a custom function call instead, as seen in column (b), line 28.

\subsubsection{Tool Limitations}
The current version of \tool does not support the custom function transformation needed to transform the API call \code{add many}.  We believe that this function could be implemented but it remains as future work.  For this reason,  \tool is unable to support transformations that involve the \code{add many} action.
