...
\TODO{Microsoft guys, will you please fill out this section.  We were thinking perhaps about a page, talking about the background of TouchDevelop, and specifically focusing on Cloud Data types.}
...
After converting these apps, we ran them with multiple users on three devices to verify that in fact the apps work correctly. \TODO{Put here the IDs of the changed apps}
...
Lines XYZ shows the declaration of the \code{MileageRecord} \NC{}, which is the target of the refactoring. We show four functions to display all mileage record,
clear existing mileage record, add a new entry to the mileage collection, and compute and display the average mileage. \\
...
\begin{figure*}
   \centering
    ...
    \caption{Selected code from Mileage Tracker app. \TODO{add the definitions of data structure, and update text with the new line numbers}}
    \label{fig:codeTransformations}
\end{figure*}

\myParagraph{Create \texttt{Cloud Data Table} data structure: }
Lines XYZ of the right-hand side of Figure~\ref{fig:codeTransformations} show the newly created data structure, \code{MileageRecordTable}. We append the \code{Table} to the end of the name for clarity. This \code{Table} contains one column, \code{MileageRecordColumn}. We will use this column to store the values that were originally stored in the \code{MileageRecord} collection.
\\
...
Figure~\ref{fig:codeTransformations}~(a) line 4 shows the API call before the transformation, while Fig.~\ref{fig:codeTransformations}~(b) line 
4 shows the code after the transformation.\\
...
The function \code{AddMileageToRecord} adds a number to the \code{MileageRecord} \NC{}. Line 22 in column (a) shows \code{Mileage} being added via the API call \code{add}.
...
This becomes a two step process. Step 1 is to add a new row to the \CDT, and step two is to assign the value of \code{Mileage} to that row. This is shown in column (b) lines 22-23. \\
...
One example of an Custom Function Transformation is shown in Fig~\ref{fig:codeTransformations}~(a) on lines 27-29.
...
of all contents of a \CDT. Instead, \tool must introduce the custom function \code{MileageRecordTable avg} (see Fig.~\ref{fig:codeTransformations}~(b) lines 32-41) that will compute the average of all the values in the \code{MileageRecordTable}.  
Now the API call in \code{DisplayAvgMileage} is transformed to call this \code{avg} custom function instead, as seen in Fig.~\ref{fig:codeTransformations}~(b), line 28.
...
\TODO{write about JSON AST nodes}
...
\MyParagraph{Cloud-related refactorings:}
Several related works describe refactorings to take advantage of some aspect of cloud computing.  One of the most common of these is to use the cloud to offload computing in order to make apps more energy efficient and thereby preserve battery life.  Some examples of this are Spectra~\cite{flinn2002balancing}, Slingshot~\cite{su2005slingshot}, and MAUI~\cite{cuervo2010maui}. 
Kwon and Tilervich proposed an approach to offload computation to the cloud to save battery life that is even tolerant to network outages or unavailability~\cite{kwon2012energy}.
...
\TODO{ASK DANNY, THIS GUY is a program committee member}
...
Many have studied how to implement Multi-user apps.  There are many different motivations for researching how multi-user systems work, including collaboration~\cite{Yuill:multiuserColab, Lopez-Gulliver:imageprocessing}, gaming~\cite{Leichtenstern:multiuserGames}, Geospatial Applications~\cite{Forlines:geospatial} (i.e. Google earth), information sharing~\cite{Nacenta:2012:LMM:2307798.2307816} and even music~\cite{Sorensen:2012:ISM:2399016.2399094}. 
...
\TD is a programming platform developed by Microsoft Research that allows hobbyist,novice programmers and end users  to program \emph{on} their phone for their phone.  There are other programming environments that are targeted to a similar audience, including:  App Inventor~\cite{Wolber}, Scratch~\cite{maloney2010scratch}, Alice~\cite{cooper2010design}, and Greenfoot~\cite{kolling2010greenfoot}.  In addition, end users have been targeted with such tools as excel~\cite{badame2012refactoring}.
