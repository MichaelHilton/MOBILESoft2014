...
Using the lessons that we learned from the formative study, we designed and implemented a refactoring tool, \tool, to automate the conversion of local \code{Number Collection} into \code{Cloud Data Table}, and to transform the API calls. We selected this refactoring because its manual application is 
...
that contained a \code{Number Collection}. We retained the top 109 scripts.

%We also required the \code{Number Collection} be a global variable since 
...
We ran the refactoring in batch mode. From each script in our corpus, our tool randomly selects one \code{Number Collection} as the target for refactoring. 
